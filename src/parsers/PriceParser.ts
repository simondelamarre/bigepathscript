...
export default (str): PRICE[] => {
  ...
  for (const price of allPrices) {
    ...
    let integer: number = parseInt(price.replace(/[^\d]/g, ""));
    /* eslint-disable */
    const devise = price.replace(/\,|\.|\d/g, "");
    if (!separator) integer = integer * 100;
    if (devise)
...
export const getDevise = (devise): string => {
  if (!devise) {
    return 'EUR';
  }
  devise = devise.replace(/\s/g, '')
  let response = devise;
  if (Object.keys(currency).includes(devise)) return devise;
  for (const cur of Object.keys(currency)) {
    if (currency[cur].indexOf(devise)) return cur;
  }
  return response;
}
